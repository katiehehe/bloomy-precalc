...
1. **How a polar graph is swept out (rose, $n = 2$).** Plot a few points of
...
2. **How to count the petals of a rose (rose, animate $n$).** Watch $n = 2, 3, 4, 5$. State the
   rule: odd $n$ gives $n$ petals, even $n$ gives $2n$. Choices on $\cos 2\theta$
   and $\cos 3\theta$.
3. **What a cardioid is.** $r = 1 + \cos\theta$: max $2$ at $\theta = 0$,
   cusp at the origin where $r = 0$ at $\theta = 180^\circ$. Choices on $r$ at
   $180^\circ$ and where the max occurs.
4. **What a limaçon is (limacon, animate $a$ with $b = 1$).** $a < b$ inner loop, $a = b$
   cardioid, $a > b$ dimple flattening to convex at $a \ge 2b$. Choices on the
   loop condition and the cardioid case.
5. **Your turn: recognize a polar graph.** Sliders set $n$ and sweep the tracer.
