...
### Unit 9 - Calculus readiness  (DONE: 5/5 gated)
...
- [x] concavity (concavity) - concave up = cup (tangents below, slope increasing, f'' > 0), concave down = cap (tangents above, slope decreasing, f'' < 0), inflection point = where concavity switches. Examples x^2 (up), -x^2 (down), x^3 (switches at 0, f''=6x), sqrt(x) (increasing yet concave down), x^4 (f''(0)=0 but no inflection). Verified: 2a slopes on x^2 give -3,0,3 (increasing); x^3 concave down for x<0, up for x>0, inflection (0,0); sqrt(x) increasing+concave down; left arm of x^2 decreasing+concave up; your-turn point rides x^3 (x=v/10) starts x=-1.5 (concave down, off answer), passes only at x=0 where f''=6x=0. 30 Q hand-verified (concave-up vs increasing, tangents-above-for-up, wrong side of x^3, every-critical-point-is-an-inflection, f''=0 guarantees inflection [x^4 counterexample]). check PASS, build PASS, smoke 3/3 clean; 5 slides shot-walked (cup+cap tangents, three 2a slopes, x^3 inflection labels, sqrt/left-parabola increasing-vs-concavity payoff color-coded, live f''=6x readout).
- [x] limits-graph (limits-graph) - a limit is the height both branches approach near x=a, independent of f(a); two-sided limit exists iff left = right; a removable hole still has a limit, a jump does not. Running graphs verified: g=(x^2-4)/(x-2)=x+2 hole (2,4), lim=4; table 1.9->3.9 / 2.1->4.1; h with h(2)=1 filled dot yet lim=4 (removable); piecewise p=x-1 (x<=2) / x+1 (x>2) left limit 1 (filled (2,1)), right limit 3 (open (2,3)), DNE. Your-turn plot: click the hole (2,4) (starts unsolved). 30 Q hand-verified (read f(a) vs limit, one-sided must be EQUAL, jump=>DNE, hole!=DNE, |x|/x DNE, divergent table). check PASS, build PASS, smoke 3/3 clean; 5 slides shot-walked (hole+table, one-sided marks, value-vs-limit, two-colour jump with y=1/y=3, your-turn hole).
- [x] limits-alg (limits-alg) - try direct substitution; 0/0 is a signal (not an answer, and distinct from nonzero/0 = asymptote) to simplify then substitute. Three methods via AlgebraFlow, all verified: factor/cancel (x^2-4)/(x-2)->x+2->4; conjugate (sqrt(x+4)-2)/x, top (x+4)-4=x, ->1/(sqrt(x+4)+2)->1/4; compound fraction (1/(x+3)-1/3)/x, top -x, ->-1/(3(x+3))->-1/9. Your-turn plot: (x^2-1)/(x-1)->2, click hole (1,2). 30 Q hand-verified (report 0/0, cancel before factor, forget to substitute, -1/9 vs 1/9 sign, drop conjugate cross term, x^3-1 factor->3, (x+2)^2-4 expand->4, x/(sqrt(x+1)-1)->2, nonzero/0 asymptote). check PASS, build PASS, smoke 3/3 clean; 5 slides shot-walked. Layout fix: the three derivation flows now render as the main (height-bounded) figure with the curve glyph as the AlgebraFlow `header`, so the flow scrolls to the newest line and the final substituted value (4, 1/4, -1/9) is always in view (in the auto-height dock the tall compound-fraction flow overflowed and hid the answer; the built-in scroll-to-newest only works when the flow is height-bounded).
- [x] continuity (continuity) - continuous at x=a needs all three: f(a) defined, two-sided limit exists, and they are equal; it fails as a removable hole, a jump, or an infinite discontinuity (wall). Verified: f=x^2/2 continuous at 2 (f(2)=2=lim); g=(x^2-4)/(x-2) hole (2,4) patch g(2)=4 (AlgebraFlow factor/cancel); piecewise x+1(x<1)/x+3(x>=1) left 2 (open) / right & f(1)=4, jump 2, DNE; 1/(x-2) wall, +inf right / -inf left, not patchable. Your-turn slider f(2)=v/10, max=50 so the 41-sample sweep lands exactly on v=40 (=4=lim, continuous), starts v=10 (f(2)=1.0, off answer). 30 Q hand-verified (keep only one of three conditions, hole has a limit, jump/infinite not removable, which condition fails, poly everywhere, rational except q=0, holes (3,6)/(-2,-4), solve k=2 for continuity, hole-vs-wall by cancellation, |x| continuous). check PASS, build PASS, smoke 3/3 clean; 5 slides shot-walked (three-condition parabola, factor/cancel hole+patch, two-colour jump, 1/(x-2) wall with +/-inf, movable patch point starting off the hole).

Unit 9 gate: full `npm run verify` PASS (eval:self + eval:rational + check 51/51 + build); smoke 15/15 clean across the 5 lessons (each lesson + climb + summit); all 25 slides shot-walked and eyeballed. CurvePlane reused by all five (dq/concavity secants+tangents, limits-graph/continuity holes/jumps/walls, limits-alg glyphs); limits-alg + continuity also use AlgebraFlow.
