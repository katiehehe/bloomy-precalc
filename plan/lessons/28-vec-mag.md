# 28. Magnitude and direction

- Skill id: `vec-mag`
- Unit: Vectors (Unit 5, block plane)
- Status: ready
- This skill is already taught by the Base Camp lesson `vectors`. The Journey links to it; do not rebuild.
...
- `vec-mag` Magnitude and direction

## Prerequisites (must already be learned)
- Unit-level: Component form is cosine and sine, so this waits on the core-trig spine. Matrices are not a prerequisite.
